...
ルール:
1. 「明日」「来週」「明後日」「今週末」などの相対的な表現は、上記の現在日時を基準に解決してください。時刻が不明な場合のみstartDateTime/endDateTimeを空文字列にしてください。開始時刻のみ記載されていて終了時刻が不明な場合は、終了時刻を開始時刻の1時間後にしてください。
2. タイムゾーンはAsia/Tokyo（+09:00）を使用してください。例: "2026-04-01T10:00:00+09:00"
3. 年が省略されている場合は現在年（${jst.getFullYear()}年）として解釈してください。
4. allDayの判定基準: コンテンツに時刻情報が一切含まれていない場合のみtrueにしてください。時刻が記載されているが読み取れない・省略されている場合はfalseにしてください。判断できない場合はnullにしてください。
5. allDay: trueの場合、startDateTime/endDateTimeは "YYYY-MM-DDT00:00:00+09:00" 形式にしてください。allDay: falseで時刻不明の場合はstartDateTime/endDateTimeを空文字列にしてください。
...
8. memoフィールドには予定の主旨を簡潔に要約して記載してください。箇条書きや冗長な表現は避け、参加・実行に必要な情報（URL・場所の詳細・持ち物など）も含めてください。
9. startDateTimeは必ずendDateTime以前の日時にしてください。開始日時が終了日時より後になってはいけません。
10. locationは企業名・施設名など具体的な場所を1つに絞って記載してください。
11. 複数日にまたがる予定（合宿・出張など）は、1日ごとに分割して個別のイベントとして抽出してください。`;
}
